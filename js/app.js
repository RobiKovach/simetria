(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new Swiper(".doctrs__slider", {
                ...
                normalizeSlideIndex: true,
                navigation: {
                    prevEl: ".doctrs-button-prev",
                    nextEl: ".doctrs-button-next"
                },
                breakpoints: {
                    375: {
                        slidesPerView: 1.3,
            ...
            new Swiper(".cases__slider2", {
                ...
                normalizeSlideIndex: true,
                navigation: {
                    prevEl: ".cases-button-prev2",
                    nextEl: ".cases-button-next2"
                },
                breakpoints: {
                    375: {
                        slidesPerView: 1.3,
    ...
    document.querySelectorAll(".actions-cases__tab").forEach(btn => {
        btn.addEventListener("click", function() {
            const tab = this.dataset.tab;
            document.querySelectorAll(".actions-cases__tab").forEach(b => b.classList.remove("_active"));
            document.querySelectorAll(".cases__slider-wrap").forEach(s => s.classList.remove("_active"));
            this.classList.add("_active");
            document.querySelector(`[data-slider="${tab}"]`).classList.add("_active");
            const isAdults = tab === "adults";
            document.querySelector(".cases-button-prev").style.display = isAdults ? "" : "none";
            document.querySelector(".cases-button-next").style.display = isAdults ? "" : "none";
            document.querySelector(".cases-button-prev2").style.display = isAdults ? "none" : "";
    ...
    document.getElementById("form-submit").addEventListener("click", async function() {
        ...
        try {
            ...
            if (res.ok) window.location.href = "thanks.html"; else throw new Error(`HTTP ${res.status}`);
        } catch (err) {
            console.error("Webhook error:", err);
            status.textContent = "Сталася помилка. Спробуйте ще раз.";
            status.style.color = "red";
            status.style.display = "block";
        } finally {
            this.disabled = false;
            this.textContent = "надіслати";
        }
    });
    const phoneInput = document.getElementById("phone");
    phoneInput.addEventListener("input", function(e) {
        let digits = this.value.replace(/\D/g, "");
        if (digits.startsWith("380")) digits = digits.slice(2); else if (digits.startsWith("38")) digits = digits.slice(2);
        digits = digits.slice(0, 10);
        let formatted = "+38 ";
        if (digits.length === 0) formatted = ""; else if (digits.length <= 3) formatted += `(${digits}`; else if (digits.length <= 6) formatted += `(${digits.slice(0, 3)}) ${digits.slice(3)}`; else if (digits.length <= 8) formatted += `(${digits.slice(0, 3)}) ${digits.slice(3, 6)}-${digits.slice(6)}`; else formatted += `(${digits.slice(0, 3)}) ${digits.slice(3, 6)}-${digits.slice(6, 8)}-${digits.slice(8)}`;
        this.value = formatted;
    });
    phoneInput.addEventListener("keydown", function(e) {
        if (e.key === "Backspace") {
            const digits = this.value.replace(/\D/g, "");
            if (digits.length <= 1) this.value = "";
        }
    });
    ...
    document.querySelectorAll("a[data-goto]").forEach(link => {
        link.addEventListener("click", function(e) {
            ...
            if (!isOnIndexPage) {
                e.stopImmediatePropagation();
                window.location.href = "index.html#" + this.dataset.goto.replace(".", "");
            }
        });
    });
    pageNavigation();
